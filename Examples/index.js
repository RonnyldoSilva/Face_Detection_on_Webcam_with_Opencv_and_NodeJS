...
const ssdcocoModelPath = './dnn/coco/SSD_300x300';
...
const runDetectCarsExample = () => {
  const img = cv.imread('./data/cars2.jpg');
...
const runDetectPeopleExample = () => {
  const img = cv.imread('./data/peopleTBBT.jpg');
  const minConfidence = 0.3;
  ...
  drawClassDetections(img, 'person', getRandomColor);
  cv.imshowWait('img', img);
};

runDetectDishesExample();
runDetectCarsExample();
